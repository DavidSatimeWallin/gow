...
gitCommits=`git rev-list --count HEAD --no-merges`
gitCount=${#gitCommits}
majorVer=0
firstVer=${gitCommits:0:1}
restVer=${gitCommits:1:12}
Version=${majorVer}.${firstVer}.${restVer}
...
for PLATFORM in $PLATFORMS; do
  GOOS=${PLATFORM%/*}
  GOARCH=${PLATFORM#*/}
  BIN_FILENAME="binaries/${OUTPUT}-${Version}-${GOOS}-${GOARCH}"
...
if [[ $PLATFORMS_ARM == *"linux"* ]]; then 
  CMD="GOOS=linux GOARCH=arm64 go build -o binaries/${OUTPUT}-${Version}-linux-arm64 $@"
  echo "${CMD}"
  eval $CMD || FAILURES="${FAILURES} ${PLATFORM}"
fi
for GOOS in $PLATFORMS_ARM; do
  ...
  for GOARM in 7 6 5; do
    ...
    CMD="GOARM=${GOARM} GOOS=${GOOS} GOARCH=${GOARCH} go build -o binaries/${BIN_FILENAME} $@"
    echo "${CMD}"
